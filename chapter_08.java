public class chapter_08 {
    ...
    //Существует несколько распространенных ошибок, связанных с разрешениями доступа к файлам:

    //777 к файлам или каталогам.
    //Это широко распространено в плохо написанных интернет-руководствах,
    //представляющих "chmod 777" как универсальный способ для решения всех проблем, связанных с правами доступа.
    //Никогда не делайте этого, если вы точно не знаете, что делаете.
    //Установка этого параметра без четкого понимания определенно
    //подвергнет этот объект высокому риску быть произвольно измененным,
    //удаленным и (для конфиденциальных данных) прочитанным любым пользователем и процессом системы и
    //продемонстрирует вашу неспособность правильно работать с UNIX-подобными операционными системами,
    //что часто приводит к отзыву доступа и другим личным проблемам.
    ...
    //Существует 3 специальных бита доступа и 1 случай, о которых необходимо знать.  Они описаны ниже.

    //Символические ссылки

    //Символические ссылки всегда имеют разрешения 777, но это ничего не значит,
    //поскольку фактические разрешения доступа такие же, как у целевого объекта:
    //lrwxrwxrwx. 1 root root 7 янв 1 21:12 /bin -> /usr/bin
    //dr-xr-xr-x. 2 root root 36864 фев 19 13:31 /usr/bin

    //В приведенном примере вы можете считать, что любой пользователь может делать все что угодно с символической ссылкой /bin.
    //Но на самом деле:
    ...
    //Может использоваться для директорий наряду с "обычными" битами доступа rwx.
    //Если он установлен, только владельцы могут удалять объекты, независимо от разрешения других (например, 777 для /tmp).
    //Sticky bit отображается как 't' в конце "слова" разрешения доступа, см. ниже "drwxrwxrwt":

    //[st00@c7-sandbox ~]$ ls -ld /tmp
    //drwxrwxrwt. 52 root root 12288 Jan 19 01:35 /tmp

    //Это еще лучше видно при использовании инструмента "stat",
    //который также подходит для отображения подробностей о конкретном объекте файловой системы (файл/директория/симлинк).

    //[st00@c7-sandbox ~]$ stat /tmp
    //  File: '/tmp'
    //  Size: 12288           Blocks: 24         IO Block: 4096   directory
    //Device: fd02h/64770d    Inode: 2           Links: 52
    //Access: (1777/drwxrwxrwt)  Uid: (    0/    root)   Gid: (    0/    root)
    //Context: system_u:object_r:tmp_t:s0
    //Access: 2023-01-18 20:04:51.593082848 +0300
    //Modify: 2023-01-19 01:37:28.406328716 +0300
    //Change: 2023-01-19 01:37:28.406328716 +0300
    // Birth: -
    //Paragraph
    //Edit

    //См. поле "Доступ" выше, оно имеет значение "(1777/drwxrwxrwt)". На самом деле, sticky bit имеет числовое значение 1.
    //Что касается /tmp, sticky bit очень важен, так как благодаря разрешениям 777 каждый может создавать и удалять там что угодно.
    //Sticky bit предотвращает анархию, не позволяя удалять чужие данные; вы можете попробовать удалить что-то из /tmp,
    //принадлежащее другому пользователю, и вы увидите, что из этого получится.

    //Он может быть установлен в восьмеричной или символьной форме, как показано ниже:

    //chmod +t /some/dir
    //chmod 1xyz /some/dir

    //Где 'xyz' обозначает точные восьмеричные разрешения.


    //Биты SUID и SGID
    //Бит SUID (Set User ID) может использоваться для исполняемых файлов.
    //Если он установлен, этот файл всегда будет выполняться от имени владельца.
    //Бит SUID отображается как 's' вместо 'x' в блоке разрешений владельца:
    ...
    //Это полезно, если этот исполняемый файл
    //а)часто используется
    //б)должен делать что-то с определенными правами пользователя.
    //Например.

    //Исполняемый файл "/bin/passwd" предназначен для того, чтобы позволить пользователям изменять свой пароль.
    //Поэтому он должен модифицировать файл /etc/shadow, что требует привилегий root.

    //Исполняемый файл "/bin/chsh" предназначен для изменения оболочки входа в систему.
    //Поэтому он модифицирует файл /etc/passwd, который также разрешен только для root.

    //Исполняемый файл "/bin/sudo" предназначен для повышения привилегий до "root",
    //поэтому он обязательно должен иметь возможность запускать что-либо от имени root.


    //Бит SGID (Set Group ID) также может быть использован для исполняемых файлов.
    //Если он установлен, этот файл всегда будет выполняться от имени группы владельцев.
    //Бит SGID отображается как 's' вместо 'x' в блоке групповых разрешений, как показано выше для бинарного файла "wall".
    //В отличие от SUID, бит SGID не используется широко.

    //Бит SUID имеет восьмеричное значение 4, а SGID - 2. Это можно проверить с помощью инструмента "stat"

    //Эти специальные биты можно установить с помощью инструмента chmod, как показано ниже:
    ...
    //Результаты урока
    //1 - Пользователи и группы предназначены для разделения привилегий
    //с целью обеспечения безопасности и стабильности ОС и ее приложений.

    //2 - Вы можете использовать инструменты "su" и (чаще) "sudo" для выполнения
    //команд с привилегиями администратора, если это разрешено

    //3 - Каждый объект файловой системы, включая файлы и директории, имеет свои собственные разрешения доступа,
    //связанные как с владельцами/группами, так и с разрешенными операциями чтения/записи/выполнения

    //4 - Одни и те же биты разрешения RWX имеют разные эффекты для файлов и директорий

    //5 - Владельцы/группы и разрешения доступа могут быть изменены с помощью инструментов chown, chgrp, chmod

    //6 - Биты доступа также могут быть записаны как восьмеричные числа.

    //7 - Имеется 3 специальных бита доступа (sticky bit, биты SUID и SGID).
